...
class CSR(HL7Model):
    ...
    csr_2 : EI | None
        CSR.2 (opt) - Alternate Study ID (EI) S8.10.2.3
    ...
    csr_5 : CX | None
        CSR.5 (opt) - Alternate Patient ID - CSR (CX) S7.7.1.5

    csr_6 : TS
        CSR.6 (req) - Date/Time Of Patient Study Registration (TS) S7.7.1.6

    csr_7 : list[XCN] | None
        CSR.7 (opt, rep) - Person Performing Study Registration (XCN) S7.7.1.7

    csr_8 : list[XCN]
        CSR.8 (req, rep) - Study Authorizing Provider (XCN) S7.7.1.8
    ...
    csr_12 : list[CE] | None
        CSR.12 (opt, rep) - Randomized Study Arm (CE) S7.7.1.12
