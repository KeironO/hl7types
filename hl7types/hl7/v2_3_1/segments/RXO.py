...
class RXO(HL7Model):
    ...
    ----------
    rxo_1 : CE | None
        RXO.1 (opt) - Requested Give Code (CE) S4.8.2.1

    rxo_2 : str | None
        RXO.2 (opt) - Requested Give Amount - Minimum (NM) S4.8.2.2

    rxo_3 : str | None
        RXO.3 (opt) - Requested Give Amount - Maximum (NM) S4.8.2.3

    rxo_4 : CE | None
        RXO.4 (opt) - Requested Give Units (CE) S4.8.2.4

    rxo_5 : CE | None
        RXO.5 (opt) - Requested Dosage Form (CE) S4.8.2.5

    rxo_6 : list[CE] | None
        RXO.6 (opt, rep) - Provider’s Pharmacy/Treatment Instructions (CE) S4.8.2.6

    rxo_7 : list[CE] | None
        RXO.7 (opt, rep) - Provider’s Administration Instructions (CE) S4.8.7.7

    rxo_8 : LA1 | None
        RXO.8 (opt) - Deliver-to Location (LA1) S4.8.7.8
    ...
    rxo_14 : list[XCN] | None
        RXO.14 (opt, rep) - Ordering Provider’s DEA Number (XCN) S4.8.7.13

    rxo_15 : list[XCN] | None
        RXO.15 (opt, rep) - Pharmacist/Treatment Supplier’s Verifier ID (XCN) S4.8.7.14

    rxo_16 : str | None
        RXO.16 (opt) - Needs Human Review (ID) S4.8.12.12 | 0136 - Yes/no indicator

    rxo_17 : str | None
        RXO.17 (opt) - Requested Give Per (Time Unit) (ST) S4.8.2.17

    rxo_18 : str | None
        RXO.18 (opt) - Requested Give Strength (NM) S4.8.2.18

    rxo_19 : CE | None
        RXO.19 (opt) - Requested Give Strength Units (CE) S4.8.2.19

    rxo_20 : list[CE] | None
        RXO.20 (opt, rep) - Indication (CE) S4.8.14.19

    rxo_21 : str | None
        RXO.21 (opt) - Requested Give Rate Amount (ST) S4.8.2.21

    rxo_22 : CE | None
        RXO.22 (opt) - Requested Give Rate Units (CE) S4.8.2.22

    rxo_23 : CQ | None
        RXO.23 (opt) - Total Daily Dose (CQ) S4.8.10.12
    """
