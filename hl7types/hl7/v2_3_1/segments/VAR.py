"""
Profile: urn:hl7-org:v2xml
Release: v2
Version: 2.3.1
Class: VAR
Type: Segment
"""
from __future__ import annotations

from typing import Optional, List
from pydantic import AliasChoices, ConfigDict, Field
from hl7types.hl7 import HL7Model

from ..datatypes.CE import CE
from ..datatypes.EI import EI
from ..datatypes.TS import TS
from ..datatypes.XCN import XCN


class VAR(HL7Model):
    """Variance (S12.3.5).

    Attributes
    ----------
    var_1 : EI
        VAR.1 - Variance Instance ID (EI) R S12.3.5.1

    var_2 : TS
        VAR.2 - Documented Date/Time (TS) R S12.3.5.2

    var_3 : TS | None
        VAR.3 - Stated Variance Date/Time (TS) O S12.3.5.3

    var_4 : list[XCN] | None
        VAR.4 - Variance Originator (XCN) O rep S12.3.5.4

    var_5 : CE | None
        VAR.5 - Variance Classification (CE) O S12.3.5.5

    var_6 : list[str] | None
        VAR.6 - Variance Description (ST) O rep S12.3.5.6
    """

    var_1: EI = Field(
        validation_alias=AliasChoices(
            "var_1",
            "variance_instance_id",
            "VAR.1",
        ),
        serialization_alias="VAR.1",
        title="Variance Instance ID",
        description="R | Item #01212",
    )

    var_2: TS = Field(
        validation_alias=AliasChoices(
            "var_2",
            "documented_date_time",
            "VAR.2",
        ),
        serialization_alias="VAR.2",
        title="Documented Date/Time",
        description="R | Item #01213",
    )

    var_3: Optional[TS] = Field(
        default=None,
        validation_alias=AliasChoices(
            "var_3",
            "stated_variance_date_time",
            "VAR.3",
        ),
        serialization_alias="VAR.3",
        title="Stated Variance Date/Time",
        description="O | Item #01214",
    )

    var_4: Optional[List[XCN]] = Field(
        default=None,
        validation_alias=AliasChoices(
            "var_4",
            "variance_originator",
            "VAR.4",
        ),
        serialization_alias="VAR.4",
        title="Variance Originator",
        description="O | Item #01215",
    )

    var_5: Optional[CE] = Field(
        default=None,
        validation_alias=AliasChoices(
            "var_5",
            "variance_classification",
            "VAR.5",
        ),
        serialization_alias="VAR.5",
        title="Variance Classification",
        description="O | Item #01216",
    )

    var_6: Optional[List[str]] = Field(
        default=None,
        validation_alias=AliasChoices(
            "var_6",
            "variance_description",
            "VAR.6",
        ),
        serialization_alias="VAR.6",
        title="Variance Description",
        description="O | Item #01217 | LEN:512",
    )

    model_config = ConfigDict(populate_by_name=True)
