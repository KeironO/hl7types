...
class RXG(HL7Model):
    ...
    rxg_3 : TQ
        RXG.3 (req) - Quantity/Timing (TQ) S7.3.1.27

    rxg_4 : CE
        RXG.4 (req) - Give Code (CE) S4.8.12.4 | 0292 - Vaccines administered
    ...
    rxg_9 : list[CE] | None
        RXG.9 (opt, rep) - Administration Notes (CE) S4.8.14.9

    rxg_10 : str | None
        RXG.10 (opt) - Substitution Status (ID) S4.8.12.10 | 0167 - Substitution status

    rxg_11 : LA2 | None
        RXG.11 (opt) - Dispense-To Location (LA2) S4.8.12.11

    rxg_12 : str | None
        RXG.12 (opt) - Needs Human Review (ID) S4.8.12.12 | 0136 - Yes/no indicator

    rxg_13 : list[CE] | None
        RXG.13 (opt, rep) - Pharmacy/Treatment Supplier’s Special Administration Instructions (CE) S4.8.12.13

    rxg_14 : str | None
        RXG.14 (opt) - Give Per (Time Unit) (ST) S4.8.12.14

    rxg_15 : str | None
        RXG.15 (opt) - Give Rate Amount (ST) S4.8.12.15

    rxg_16 : CE | None
        RXG.16 (opt) - Give Rate Units (CE) S4.8.12.16

    rxg_17 : str | None
        RXG.17 (opt) - Give Strength (NM) S4.8.12.17

    rxg_18 : CE | None
        RXG.18 (opt) - Give Strength Units (CE) S4.8.12.18

    rxg_19 : list[str] | None
        RXG.19 (opt, rep) - Substance Lot Number (ST) S4.8.14.15

    rxg_20 : list[TS] | None
        RXG.20 (opt, rep) - Substance Expiration Date (TS) S4.8.14.16

    rxg_21 : list[CE] | None
        RXG.21 (opt, rep) - Substance Manufacturer Name (CE) S4.8.14.17 | 0227 - Manufacturers of vaccines (code=MVX)

    rxg_22 : list[CE] | None
        RXG.22 (opt, rep) - Indication (CE) S4.8.14.19
    """
