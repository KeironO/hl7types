...
class LRL(HL7Model):
    """LRL - location relationship segment (S8.8.4).

    Attributes
    ----------
    lrl_1 : PL
        LRL.1 - Primary Key Value - LRL (PL) R S8.8.4.1

    lrl_2 : str | None
        LRL.2 - Segment Action Code (ID) O S10.5.7.2 | 0206 - Segment action code

    lrl_3 : EI | None
        LRL.3 - Segment Unique Key (EI) O S8.8.4.3

    lrl_4 : CE | None
        LRL.4 - Location Relationship ID (CE) O S8.8.4.4 | 0325 - Location relationship ID

    lrl_5 : list[XON] | None
        LRL.5 - Organizational Location Relationship Value (XON) C rep S8.8.4.5

    lrl_6 : PL | None
        LRL.6 - Patient Location Relationship Value (PL) C S8.8.4.6
    """
    ...
    lrl_4: Optional[CE] = Field(
        ...
        description="O | Item #01277 | Table 0325 - Location relationship ID",
    )
