...
class LRL(HL7Model):
    ...
    ----------
    lrl_1 : PL
        LRL.1 (req) - Primary Key Value - LRL (PL) S8.8.4.1

    lrl_2 : str | None
        LRL.2 (opt) - Segment Action Code (ID) S10.5.7.2 | 0206 - Segment action code

    lrl_3 : EI | None
        LRL.3 (opt) - Segment Unique Key (EI) S8.8.4.3

    lrl_4 : CE | None
        LRL.4 (opt) - Location Relationship ID (CE) S8.8.4.4 | 0325 - Location relationship ID

    lrl_5 : list[XON] | None
        LRL.5 (opt, rep) - Organizational Location Relationship Value (XON) S8.8.4.5

    lrl_6 : PL | None
        LRL.6 (opt) - Patient Location Relationship Value (PL) S8.8.4.6
    """
