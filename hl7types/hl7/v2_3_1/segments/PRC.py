...
class PRC(HL7Model):
    """PRC -  pricing segment (S8.9.3).

    Attributes
    ----------
    prc_1 : CE
        PRC.1 - Primary Key Value - PRC (CE) R S8.9.3.1 | 0132 - Transaction Code

    prc_2 : list[CE] | None
        PRC.2 - Facility ID - PRC (CE) O rep S8.9.3.2

    prc_3 : list[CE] | None
        PRC.3 - Department (CE) O rep S8.9.3.3 | 0184 - Department

    prc_4 : list[str] | None
        PRC.4 - Valid Patient Classes (IS) O rep S8.9.3.4 | 0004 - Patient class

    prc_5 : list[CP] | None
        PRC.5 - Price (CP) C rep S8.9.3.5

    prc_6 : list[str] | None
        PRC.6 - Formula (ST) O rep S8.9.3.6

    prc_7 : str | None
        PRC.7 - Minimum Quantity (NM) O S8.9.3.7

    prc_8 : str | None
        PRC.8 - Maximum Quantity (NM) O S8.9.3.8

    prc_9 : MO | None
        PRC.9 - Minimum Price (MO) O S8.9.3.9

    prc_10 : MO | None
        PRC.10 - Maximum Price (MO) O S8.9.3.10

    prc_11 : TS | None
        PRC.11 - Effective Start Date (TS) O S8.9.3.11

    prc_12 : TS | None
        PRC.12 - Effective End Date (TS) O S8.9.3.12

    prc_13 : str | None
        PRC.13 - Price Override Flag (IS) O S8.9.3.13 | 0268 - Override

    prc_14 : list[CE] | None
        PRC.14 - Billing Category (CE) O rep S8.9.3.14 | 0293 - Billing Category

    prc_15 : str | None
        PRC.15 - Chargeable Flag (ID) O S8.9.3.15 | 0136 - Yes/no indicator

    prc_16 : str | None
        PRC.16 - Active/Inactive Flag (ID) O S8.9.3.16 | 0183 - Active/inactive

    prc_17 : MO | None
        PRC.17 - Cost (MO) O S8.9.3.17

    prc_18 : str | None
        PRC.18 - Charge On Indicator (IS) O S8.9.3.18 | 0269 - Charge on indicator
    """
