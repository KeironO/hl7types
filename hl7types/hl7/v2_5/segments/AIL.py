...
class AIL(HL7Model):
    ...
    ail_2 : str | None
        AIL.2 (opt) - Segment Action Code (ID) S8.9.3.2 | 0206 - Segment action code

    ail_3 : list[PL] | None
        AIL.3 (opt, rep) - Location Resource ID (PL) S10.6.6.3

    ail_4 : CE | None
        AIL.4 (opt) - Location Type-AIL (CE) S10.6.6.4 | 0305 - Person location type

    ail_5 : CE | None
        AIL.5 (opt) - Location Group (CE) S10.6.6.5

    ail_6 : TS | None
        AIL.6 (opt) - Start Date/Time (TS) S10.6.4.4

    ail_7 : str | None
        AIL.7 (opt) - Start Date/Time Offset (NM) S10.6.4.5

    ail_8 : CE | None
        AIL.8 (opt) - Start Date/Time Offset Units (CE) S10.6.4.6

    ail_9 : str | None
        AIL.9 (opt) - Duration (NM) S10.6.4.7

    ail_10 : CE | None
        AIL.10 (opt) - Duration Units (CE) S10.6.4.8

    ail_11 : str | None
        AIL.11 (opt) - Allow Substitution Code (IS) S10.6.4.9 | 0279 - Allow Substitution Codes

    ail_12 : CE | None
        AIL.12 (opt) - Filler Status Code (CE) S10.6.2.25 | 0278 - Filler status codes
    """
