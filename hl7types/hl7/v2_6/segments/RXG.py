...
class RXG(HL7Model):
    ...
    rxg_4 : CWE
        RXG.4 (req) - Give Code (CWE) S4.14.4.2 | 0292 - Vaccines administered
    ...
    rxg_7 : CWE
        RXG.7 (req) - Give Units (CWE) S4.14.4.5 | 9999 - no table for CE

    rxg_8 : CWE | None
        RXG.8 (opt) - Give Dosage Form (CWE) S4.14.4.6 | 9999 - no table for CE

    rxg_9 : list[CWE] | None
        RXG.9 (opt, rep) - Administration Notes (CWE) S4.14.6.9 | 9999 - no table for CE
    ...
    rxg_13 : list[CWE] | None
        RXG.13 (opt, rep) - Pharmacy/Treatment Supplier's Special Administration Instructions (CWE) S4.14.6.13 | 9999 - no table for CE
    ...
    rxg_16 : CWE | None
        RXG.16 (opt) - Give Rate Units (CWE) S4.14.4.24 | 9999 - no table for CE

    rxg_17 : str | None
        RXG.17 (opt) - Give Strength (NM) S4.14.4.25

    rxg_18 : CWE | None
        RXG.18 (opt) - Give Strength Units (CWE) S4.14.4.26 | 9999 - no table for CE

    rxg_19 : list[str] | None
        RXG.19 (opt, rep) - Substance Lot Number (ST) S13.4.11.2

    rxg_20 : list[str] | None
        RXG.20 (opt, rep) - Substance Expiration Date (DTM) S4.14.5.19

    rxg_21 : list[CWE] | None
        RXG.21 (opt, rep) - Substance Manufacturer Name (CWE) S4.14.5.20 | 0227 - Manufacturers of Vaccines (code=MVX)

    rxg_22 : list[CWE] | None
        RXG.22 (opt, rep) - Indication (CWE) S4.14.1.20 | 9999 - no table for CE

    rxg_23 : str | None
        RXG.23 (opt) - Give Drug Strength Volume (NM) S4.14.6.23

    rxg_24 : CWE | None
        RXG.24 (opt) - Give Drug Strength Volume Units (CWE) S4.14.6.24 | 9999 - no table for CE

    rxg_25 : CWE | None
        RXG.25 (opt) - Give Barcode Identifier (CWE) S4.14.6.25 | 9999 - no table for CE

    rxg_26 : str | None
        RXG.26 (opt) - Pharmacy Order Type (ID) S4.14.6.26 | 0480 - Pharmacy Order Types

    rxg_27 : CWE | None
        RXG.27 (opt) - Dispense to Pharmacy (CWE) S4.14.5.30 | 9999 - no table for CE

    rxg_28 : XAD | None
        RXG.28 (opt) - Dispense to Pharmacy Address (XAD) S4.14.5.31

    rxg_29 : PL | None
        RXG.29 (opt) - Deliver-to Patient Location (PL) S4.14.1.34

    rxg_30 : XAD | None
        RXG.30 (opt) - Deliver-to Address (XAD) S4.14.1.35
    """
