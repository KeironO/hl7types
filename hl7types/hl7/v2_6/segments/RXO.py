...
class RXO(HL7Model):
    ...
    ----------
    rxo_1 : CWE | None
        RXO.1 - Requested Give Code (CWE) C S4.14.1.1 | 9999 - no table for CE
    ...
    rxo_4 : CWE | None
        RXO.4 - Requested Give Units (CWE) C S4.14.1.4 | 9999 - no table for CE

    rxo_5 : CWE | None
        RXO.5 - Requested Dosage Form (CWE) C S4.14.1.5 | 9999 - no table for CE

    rxo_6 : list[CWE] | None
        RXO.6 - Provider's Pharmacy/Treatment Instructions (CWE) O rep S4.14.1.6 | 9999 - no table for CE

    rxo_7 : list[CWE] | None
        RXO.7 - Provider's Administration Instructions (CWE) O rep S4.14.1.7 | 9999 - no table for CE
    ...
    rxo_10 : CWE | None
        RXO.10 - Requested Dispense Code (CWE) O S4.14.1.10 | 9999 - no table for CE

    rxo_11 : str | None
        RXO.11 - Requested Dispense Amount (NM) O S4.14.1.11

    rxo_12 : CWE | None
        RXO.12 - Requested Dispense Units (CWE) O S4.14.1.12 | 9999 - no table for CE
    ...
    rxo_19 : CWE | None
        RXO.19 - Requested Give Strength Units (CWE) O S4.14.1.19 | 9999 - no table for CE

    rxo_20 : list[CWE] | None
        RXO.20 - Indication (CWE) O rep S4.14.1.20 | 9999 - no table for CE

    rxo_21 : str | None
        RXO.21 - Requested Give Rate Amount (ST) O S4.14.1.21

    rxo_22 : CWE | None
        RXO.22 - Requested Give Rate Units (CWE) O S4.14.1.22 | 9999 - no table for CE

    rxo_23 : CQ | None
        RXO.23 - Total Daily Dose (CQ) O S4.14.1.23

    rxo_24 : list[CWE] | None
        RXO.24 - Supplementary Code (CWE) O rep S4.14.1.24 | 9999 - no table for CE

    rxo_25 : str | None
        RXO.25 - Requested Drug Strength Volume (NM) O S4.14.1.25

    rxo_26 : CWE | None
        RXO.26 - Requested Drug Strength Volume Units (CWE) O S4.14.1.26 | 9999 - no table for CE
    ...
    rxo_29 : EI | None
        RXO.29 - Medication Instance Identifier (EI) O S4.14.1.29

    rxo_30 : EI | None
        RXO.30 - Segment Instance Identifier (EI) O S4.14.1.30

    rxo_31 : CNE | None
        RXO.31 - Mood Code (CNE) C S4.14.1.31 | 0725 - Mood Codes

    rxo_32 : CWE | None
        RXO.32 - Dispensing Pharmacy (CWE) O S4.14.1.32 | 9999 - no table for CE

    rxo_33 : XAD | None
        RXO.33 - Dispensing Pharmacy Address (XAD) O S4.14.1.33

    rxo_34 : PL | None
        RXO.34 - Deliver-to Patient Location (PL) O S4.14.1.34

    rxo_35 : XAD | None
        RXO.35 - Deliver-to Address (XAD) O S4.14.1.35
    """
