...
class RXD(HL7Model):
    ...
    ----------
    rxd_1 : str
        RXD.1 - Dispense Sub-ID Counter (NM) R S4.A.4.4.1

    rxd_2 : CWE
        RXD.2 - Dispense/Give Code (CWE) R S4.A.4.4.2 | 0292 - Vaccines administered

    rxd_3 : str
        RXD.3 - Date/Time Dispensed (DTM) R S4.A.4.4.3

    rxd_4 : str
        RXD.4 - Actual Dispense Amount (NM) R S4.A.4.4.4

    rxd_5 : CWE | None
        RXD.5 - Actual Dispense Units (CWE) C S4.A.4.4.5 | 9999 - no table for CE

    rxd_6 : CWE | None
        RXD.6 - Actual Dosage Form (CWE) O S4.A.4.4.6 | 9999 - no table for CE

    rxd_7 : str
        RXD.7 - Prescription Number (ST) R S4.A.4.3.15

    rxd_8 : str | None
        RXD.8 - Number of Refills Remaining (NM) C S4.A.4.3.16

    rxd_9 : list[str] | None
        RXD.9 - Dispense Notes (ST) O rep S4.A.4.4.9

    rxd_10 : list[XCN] | None
        RXD.10 - Dispensing Provider (XCN) O rep S4.A.4.4.10

    rxd_11 : str | None
        RXD.11 - Substitution Status (ID) O S4.A.4.3.9 | 0167 - Substitution Status

    rxd_12 : CQ | None
        RXD.12 - Total Daily Dose (CQ) O S4.A.4.0.23

    rxd_13 : LA2 | None
        RXD.13 - Dispense-to Location (LA2) O S4.A.4.4.13

    rxd_14 : str | None
        RXD.14 - Needs Human Review (ID) O S4.A.4.0.16 | 0136 - Yes/no Indicator

    rxd_15 : list[CWE] | None
        RXD.15 - Pharmacy/Treatment Supplier's Special Dispensing Instructions (CWE) O rep S4.A.4.3.21 | 9999 - no table for CE

    rxd_16 : str | None
        RXD.16 - Actual Strength (NM) O S4.A.4.4.16

    rxd_17 : CWE | None
        RXD.17 - Actual Strength Unit (CWE) O S4.A.4.4.17 | 9999 - no table for CE

    rxd_18 : list[str] | None
        RXD.18 - Substance Lot Number (ST) O rep S13.4.11.2

    rxd_19 : list[str] | None
        RXD.19 - Substance Expiration Date (DTM) O rep S4.A.4.4.19

    rxd_20 : list[CWE] | None
        RXD.20 - Substance Manufacturer Name (CWE) O rep S4.A.4.4.20 | 0227 - Manufacturers of Vaccines (code=MVX)

    rxd_21 : list[CWE] | None
        RXD.21 - Indication (CWE) O rep S4.A.4.0.20 | 9999 - no table for CE

    rxd_22 : str | None
        RXD.22 - Dispense Package Size (NM) O S4.A.4.3.28

    rxd_23 : CWE | None
        RXD.23 - Dispense Package Size Unit (CWE) O S4.A.4.3.29 | 9999 - no table for CE

    rxd_24 : str | None
        RXD.24 - Dispense Package Method (ID) O S4.A.4.3.30 | 0321 - Dispense Method

    rxd_25 : list[CWE] | None
        RXD.25 - Supplementary Code (CWE) O rep S4.A.4.0.24 | 9999 - no table for CE

    rxd_26 : CWE | None
        RXD.26 - Initiating Location (CWE) O S4.A.4.4.26 | 9999 - no table for CE

    rxd_27 : CWE | None
        RXD.27 - Packaging/Assembly Location (CWE) O S4.A.4.4.27 | 9999 - no table for CE

    rxd_28 : str | None
        RXD.28 - Actual Drug Strength Volume (NM) O S4.A.4.4.28

    rxd_29 : CWE | None
        RXD.29 - Actual Drug Strength Volume Units (CWE) O S4.A.4.4.29 | 9999 - no table for CE

    rxd_30 : CWE | None
        RXD.30 - Dispense to Pharmacy (CWE) O S4.A.4.4.30 | 9999 - no table for CE

    rxd_31 : XAD | None
        RXD.31 - Dispense to Pharmacy Address (XAD) O S4.A.4.4.31

    rxd_32 : str | None
        RXD.32 - Pharmacy Order Type (ID) O S4.A.4.4.32 | 0480 - Pharmacy Order Types

    rxd_33 : CWE | None
        RXD.33 - Dispense Type (CWE) O S4.A.4.4.33 | 0484 - Dispense Type

    rxd_34 : list[XTN] | None
        RXD.34 - Pharmacy Phone Number (XTN) O rep S4.A.4.4.34
    """
