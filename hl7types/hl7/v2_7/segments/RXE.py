...
class RXE(HL7Model):
    ...
    ----------
    rxe_2 : CWE
        RXE.2 - Give Code (CWE) R S4.A.4.3.2 | 0292 - Vaccines administered

    rxe_3 : str
        RXE.3 - Give Amount - Minimum (NM) R S4.A.4.3.3

    rxe_4 : str | None
        RXE.4 - Give Amount - Maximum (NM) O S4.A.4.3.4

    rxe_5 : CWE
        RXE.5 - Give Units (CWE) R S4.A.4.3.5 | 9999 - no table for CE

    rxe_6 : CWE | None
        RXE.6 - Give Dosage Form (CWE) O S4.A.4.3.6 | 9999 - no table for CE

    rxe_7 : list[CWE] | None
        RXE.7 - Provider's Administration Instructions (CWE) O rep S4.A.4.0.7 | 9999 - no table for CE

    rxe_9 : str | None
        RXE.9 - Substitution Status (ID) O S4.A.4.3.9 | 0167 - Substitution Status

    rxe_10 : str | None
        RXE.10 - Dispense Amount (NM) C S4.A.4.3.10

    rxe_11 : CWE | None
        RXE.11 - Dispense Units (CWE) C S4.A.4.3.11 | 9999 - no table for CE

    rxe_12 : str | None
        RXE.12 - Number Of Refills (NM) O S4.A.4.0.13

    rxe_13 : list[XCN] | None
        RXE.13 - Ordering Provider's DEA Number (XCN) O rep S4.A.4.0.14

    rxe_14 : list[XCN] | None
        RXE.14 - Pharmacist/Treatment Supplier's Verifier ID (XCN) O rep S4.A.4.0.15

    rxe_15 : str | None
        RXE.15 - Prescription Number (ST) C S4.A.4.3.15

    rxe_16 : str | None
        RXE.16 - Number of Refills Remaining (NM) C S4.A.4.3.16

    rxe_17 : str | None
        RXE.17 - Number of Refills/Doses Dispensed (NM) C S4.A.4.3.17

    rxe_18 : str | None
        RXE.18 - D/T of Most Recent Refill or Dose Dispensed (DTM) C S4.A.4.3.18

    rxe_19 : CQ | None
        RXE.19 - Total Daily Dose (CQ) C S4.A.4.0.23

    rxe_20 : str | None
        RXE.20 - Needs Human Review (ID) O S4.A.4.0.16 | 0136 - Yes/no Indicator

    rxe_21 : list[CWE] | None
        RXE.21 - Pharmacy/Treatment Supplier's Special Dispensing Instructions (CWE) O rep S4.A.4.3.21 | 9999 - no table for CE

    rxe_22 : str | None
        RXE.22 - Give Per (Time Unit) (ST) C S4.A.4.3.22

    rxe_23 : str | None
        RXE.23 - Give Rate Amount (ST) O S4.A.4.3.23

    rxe_24 : CWE | None
        RXE.24 - Give Rate Units (CWE) O S4.A.4.3.24 | 9999 - no table for CE

    rxe_25 : str | None
        RXE.25 - Give Strength (NM) O S4.A.4.3.25

    rxe_26 : CWE | None
        RXE.26 - Give Strength Units (CWE) O S4.A.4.3.26 | 9999 - no table for CE

    rxe_27 : list[CWE] | None
        RXE.27 - Give Indication (CWE) O rep S4.A.4.3.27 | 9999 - no table for CE

    rxe_28 : str | None
        RXE.28 - Dispense Package Size (NM) O S4.A.4.3.28

    rxe_29 : CWE | None
        RXE.29 - Dispense Package Size Unit (CWE) O S4.A.4.3.29 | 9999 - no table for CE

    rxe_30 : str | None
        RXE.30 - Dispense Package Method (ID) O S4.A.4.3.30 | 0321 - Dispense Method

    rxe_31 : list[CWE] | None
        RXE.31 - Supplementary Code (CWE) O rep S4.A.4.0.24 | 9999 - no table for CE

    rxe_32 : str | None
        RXE.32 - Original Order Date/Time (DTM) O S4.A.4.3.32

    rxe_33 : str | None
        RXE.33 - Give Drug Strength Volume (NM) O S4.A.4.3.33

    rxe_34 : CWE | None
        RXE.34 - Give Drug Strength Volume Units (CWE) O S4.A.4.3.34 | 9999 - no table for CE

    rxe_35 : CWE | None
        RXE.35 - Controlled Substance Schedule (CWE) O S4.A.4.3.35 | 0477 - Controlled Substance Schedule*

    rxe_36 : str | None
        RXE.36 - Formulary Status (ID) O S4.A.4.3.36 | 0478 - Formulary Status

    rxe_37 : list[CWE] | None
        RXE.37 - Pharmaceutical Substance Alternative (CWE) O rep S4.A.4.3.37 | 9999 - no table for CE

    rxe_38 : CWE | None
        RXE.38 - Pharmacy of Most Recent Fill (CWE) O S4.A.4.3.38 | 9999 - no table for CE

    rxe_39 : str | None
        RXE.39 - Initial Dispense Amount (NM) O S4.A.4.3.39

    rxe_40 : CWE | None
        RXE.40 - Dispensing Pharmacy (CWE) O S4.A.4.0.32 | 9999 - no table for CE

    rxe_41 : XAD | None
        RXE.41 - Dispensing Pharmacy Address (XAD) O S4.A.4.0.33

    rxe_42 : PL | None
        RXE.42 - Deliver-to Patient Location (PL) O S4.A.4.0.34

    rxe_43 : XAD | None
        RXE.43 - Deliver-to Address (XAD) O S4.A.4.0.35

    rxe_44 : str | None
        RXE.44 - Pharmacy Order Type (ID) O S4.A.4.3.44 | 0480 - Pharmacy Order Types

    rxe_45 : list[XTN] | None
        RXE.45 - Pharmacy Phone Number (XTN) O rep S4.A.4.3.45
    """
