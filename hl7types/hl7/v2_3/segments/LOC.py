"""
Profile: urn:hl7-org:v2xml
Release: v2
Version: 2.3
Class: LOC
Type: Segment
"""
from __future__ import annotations

from typing import Optional, List
from pydantic import AliasChoices, BaseModel, Field

from ..datatypes.CE import CE
from ..datatypes.PL import PL
from ..datatypes.XAD import XAD
from ..datatypes.XON import XON
from ..datatypes.XTN import XTN


class LOC(BaseModel):
    """HL7 v2 LOC segment.

    Attributes
    ----------
    loc_1 : PL
        LOC.1 (req) - Primary Key Value (PL)

    loc_2 : str | None
        LOC.2 (opt) - Location Description (ST)

    loc_3 : list[str]
        LOC.3 (req, rep) - Location Type (IS)

    loc_4 : XON | None
        LOC.4 (opt) - Organization Name (XON)

    loc_5 : XAD | None
        LOC.5 (opt) - Location Address (XAD)

    loc_6 : list[XTN] | None
        LOC.6 (opt, rep) - Location Phone (XTN)

    loc_7 : list[CE] | None
        LOC.7 (opt, rep) - License Number (CE)

    loc_8 : list[str] | None
        LOC.8 (opt, rep) - Location Equipment (ID)
    """

    loc_1: PL = Field(
        default=...,
        validation_alias=AliasChoices(
            "loc_1",
            "primary_key_value",
            "LOC.1",
        ),
        serialization_alias="LOC.1",
        title="Primary Key Value",
        description="Item #943",
    )

    loc_2: Optional[str] = Field(
        default=None,
        validation_alias=AliasChoices(
            "loc_2",
            "location_description",
            "LOC.2",
        ),
        serialization_alias="LOC.2",
        title="Location Description",
        description="Item #944",
    )

    loc_3: List[str] = Field(
        default=...,
        validation_alias=AliasChoices(
            "loc_3",
            "location_type",
            "LOC.3",
        ),
        serialization_alias="LOC.3",
        title="Location Type",
        description="Item #945 | Table HL70260",
    )

    loc_4: Optional[XON] = Field(
        default=None,
        validation_alias=AliasChoices(
            "loc_4",
            "organization_name",
            "LOC.4",
        ),
        serialization_alias="LOC.4",
        title="Organization Name",
        description="Item #947",
    )

    loc_5: Optional[XAD] = Field(
        default=None,
        validation_alias=AliasChoices(
            "loc_5",
            "location_address",
            "LOC.5",
        ),
        serialization_alias="LOC.5",
        title="Location Address",
        description="Item #948",
    )

    loc_6: Optional[List[XTN]] = Field(
        default=None,
        validation_alias=AliasChoices(
            "loc_6",
            "location_phone",
            "LOC.6",
        ),
        serialization_alias="LOC.6",
        title="Location Phone",
        description="Item #949",
    )

    loc_7: Optional[List[CE]] = Field(
        default=None,
        validation_alias=AliasChoices(
            "loc_7",
            "license_number",
            "LOC.7",
        ),
        serialization_alias="LOC.7",
        title="License Number",
        description="Item #951",
    )

    loc_8: Optional[List[str]] = Field(
        default=None,
        validation_alias=AliasChoices(
            "loc_8",
            "location_equipment",
            "LOC.8",
        ),
        serialization_alias="LOC.8",
        title="Location Equipment",
        description="Item #953 | Table HL70261",
    )

    model_config = {"populate_by_name": True}
