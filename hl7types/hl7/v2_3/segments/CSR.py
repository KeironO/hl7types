...
class CSR(HL7Model):
    """Clinical Study Registration (S7.7.1).

    Attributes
    ----------
    csr_1 : EI
        CSR.1 (req) - Sponsor Study ID (EI) S8.10.2.2

    csr_2 : EI | None
        CSR.2 (opt) - Alternate Study ID (EI) S7.7.1.2

    csr_3 : CE | None
        CSR.3 (opt) - Institution Registering the Patient (CE) S7.7.1.3

    csr_4 : CX
        CSR.4 (req) - Sponsor Patient ID (CX) S7.7.1.4

    csr_5 : CX | None
        CSR.5 (opt) - Alternate Patient ID (CX) S7.7.1.5

    csr_6 : TS | None
        CSR.6 (opt) - Date/Time of Patient Study Registration (TS) S7.7.1.6

    csr_7 : XCN | None
        CSR.7 (opt) - Person Performing Study Registration (XCN) S7.7.1.7

    csr_8 : XCN
        CSR.8 (req) - Study Authorizing Provider (XCN) S7.7.1.8

    csr_9 : TS | None
        CSR.9 (opt) - Date/time Patient Study Consent Signed (TS) S7.7.1.9

    csr_10 : CE | None
        CSR.10 (opt) - Patient Study Eligibility Status (CE) S7.7.1.10

    csr_11 : list[TS] | None
        CSR.11 (opt, rep) - Study Randomization Date/time (TS) S7.7.1.11

    csr_12 : list[CE] | None
        CSR.12 (opt, rep) - Study Randomized Arm (CE) S7.7.1.12

    csr_13 : list[CE] | None
        CSR.13 (opt, rep) - Stratum for Study Randomization (CE) S7.7.1.13

    csr_14 : CE | None
        CSR.14 (opt) - Patient Evaluability Status (CE) S7.7.1.14

    csr_15 : TS | None
        CSR.15 (opt) - Date/time Ended Study (TS) S7.7.1.15

    csr_16 : CE | None
        CSR.16 (opt) - Reason Ended Study (CE) S7.7.1.16
    """
    ...
    csr_12: Optional[List[CE]] = Field(
        ...
        validation_alias=AliasChoices(
            "csr_12",
            "study_randomized_arm",
            "CSR.12",
        ),
        serialization_alias="CSR.12",
        title="Study Randomized Arm",
