...
class RXD(HL7Model):
    ...
    ----------
    rxd_1 : str
        RXD.1 - Dispense Sub-ID Counter (NM) R S4.8.10
    ...
    rxd_7 : str
        RXD.7 - Prescription Number (ST) R S4.8.7

    rxd_8 : str | None
        RXD.8 - Number of Refills Remaining (NM) C S4.8.7

    rxd_9 : list[CE] | None
        RXD.9 - Dispense Notes (CE) C rep S4.8.10.9

    rxd_10 : XCN | None
        RXD.10 - Dispensing Provider (XCN) O S4.8.10.10

    rxd_11 : str | None
        RXD.11 - Substitution Status (ID) O S4.8.7 | 0167 - Substitution Status

    rxd_12 : CQ | None
        RXD.12 - Total Daily Dose (CQ) C S4.8.7

    rxd_13 : str | None
        RXD.13 - Dispense-To Location (CM) O S4.8.10

    rxd_14 : str | None
        RXD.14 - Needs Human Review (ID) O S4.8.2 | 0136 - Yes/No Indicator

    rxd_15 : list[CE] | None
        RXD.15 - Pharmacy/Treatment Supplier's Special Dispensing Instructions (CE) O rep S4.8.7
    ...
    rxd_18 : list[str] | None
        RXD.18 - Substance Lot Number (ST) O rep S4.8.10

    rxd_19 : list[TS] | None
        RXD.19 - Substance Expiration Date (TS) O rep S4.8.10

    rxd_20 : list[CE] | None
        RXD.20 - Substance Manufacturer Name (CE) O rep S4.8.10 | 0227 - Manufacturers of Vaccines

    rxd_21 : CE | None
        RXD.21 - Indication (CE) O S4.8.2

    rxd_22 : str | None
        RXD.22 - Dispense Package Size (NM) O S4.8.7

    rxd_23 : CE | None
        RXD.23 - Dispense Package Size Unit (CE) O S4.8.7

    rxd_24 : str | None
        RXD.24 - Dispense Package Method (ID) O S4.8.7 | 0321 - Dispense Method
    """
