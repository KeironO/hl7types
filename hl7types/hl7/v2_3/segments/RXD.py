...
class RXD(HL7Model):
    """Pharmacy dispense segment (S4.8.10).

    Attributes
    ----------
    rxd_1 : str
        RXD.1 (req) - Dispense Sub-ID Counter (NM) S4.8.10

    rxd_2 : CE
        RXD.2 (req) - Dispense/Give Code (CE) S4.8.10.2 | 0292 - Vaccines Administered

    rxd_3 : TS
        RXD.3 (req) - Date/Time Dispensed (TS) S4.8.10.3
    ...
    rxd_7 : str
        RXD.7 (req) - Prescription Number (ST) S4.8.7

    rxd_8 : str | None
        RXD.8 (opt) - Number of Refills Remaining (NM) S4.8.7

    rxd_9 : list[CE] | None
        RXD.9 (opt, rep) - Dispense Notes (CE) S4.8.10.9

    rxd_10 : XCN | None
        RXD.10 (opt) - Dispensing Provider (XCN) S4.8.10.10

    rxd_11 : str | None
        RXD.11 (opt) - Substitution Status (ID) S4.8.7 | 0167 - Substitution Status

    rxd_12 : CQ | None
        RXD.12 (opt) - Total Daily Dose (CQ) S4.8.7

    rxd_13 : str | None
        RXD.13 (opt) - Dispense-To Location (CM) S4.8.10

    rxd_14 : str | None
        RXD.14 (opt) - Needs Human Review (ID) S4.8.2 | 0136 - Yes/No Indicator

    rxd_15 : list[CE] | None
        RXD.15 (opt, rep) - Pharmacy/Treatment Supplier's Special Dispensing Instructions (CE) S4.8.7

    rxd_16 : str | None
        RXD.16 (opt) - Actual Strength (NM) S4.8.10.16

    rxd_17 : CE | None
        RXD.17 (opt) - Actual Strength Unit (CE) S4.8.10.17

    rxd_18 : list[str] | None
        RXD.18 (opt, rep) - Substance Lot Number (ST) S4.8.10

    rxd_19 : list[TS] | None
        RXD.19 (opt, rep) - Substance Expiration Date (TS) S4.8.10

    rxd_20 : list[CE] | None
        RXD.20 (opt, rep) - Substance Manufacturer Name (CE) S4.8.10 | 0227 - Manufacturers of Vaccines

    rxd_21 : CE | None
        RXD.21 (opt) - Indication (CE) S4.8.2

    rxd_22 : str | None
        RXD.22 (opt) - Dispense Package Size (NM) S4.8.7

    rxd_23 : CE | None
        RXD.23 (opt) - Dispense Package Size Unit (CE) S4.8.7

    rxd_24 : str | None
        RXD.24 (opt) - Dispense Package Method (ID) S4.8.7 | 0321 - Dispense Method
    """
