...
class OBR(HL7Model):
    ...
    ----------
    obr_1 : str | None
        OBR.1 - Set ID - OBR (SI) O S4.4.3.1
    ...
    obr_7 : str | None
        OBR.7 - Observation Date/Time # (DTM) C S4.4.3.7

    obr_8 : str | None
        OBR.8 - Observation End Date/Time # (DTM) O S4.4.3.8

    obr_9 : CQ | None
        OBR.9 - Collection Volume * (CQ) O S4.4.3.9

    obr_10 : list[XCN] | None
        OBR.10 - Collector Identifier * (XCN) O rep S4.4.3.10

    obr_11 : str | None
        OBR.11 - Specimen Action Code * (ID) O S4.4.3.11 | 0065 - Specimen Action Code

    obr_12 : CWE | None
        OBR.12 - Danger Code (CWE) O S4.4.3.12 | 9999 - no table for CE

    obr_13 : list[CWE] | None
        OBR.13 - Relevant Clinical Information (CWE) O rep S4.4.3.13 | 0916 - Relevant Clincial Information

    obr_16 : list[XCN] | None
        OBR.16 - Ordering Provider (XCN) O rep S4.4.1.12

    obr_17 : list[XTN] | None
        OBR.17 - Order Callback Phone Number (XTN) O rep S4.4.3.17

    obr_18 : str | None
        OBR.18 - Placer Field 1 (ST) O S4.4.3.18

    obr_19 : str | None
        OBR.19 - Placer Field 2 (ST) O S4.4.3.19

    obr_20 : str | None
        OBR.20 - Filler Field 1 + (ST) O S4.4.3.20

    obr_21 : str | None
        OBR.21 - Filler Field 2 + (ST) O S4.4.3.21

    obr_22 : str | None
        OBR.22 - Results Rpt/Status Chng - Date/Time + (DTM) C S4.4.3.22

    obr_23 : MOC | None
        OBR.23 - Charge to Practice + (MOC) O S4.4.3.23

    obr_24 : str | None
        OBR.24 - Diagnostic Serv Sect ID (ID) O S4.4.3.24 | 0074 - Diagnostic Service Section ID

    obr_25 : str | None
        OBR.25 - Result Status + (ID) C S4.4.3.25 | 0123 - Result Status

    obr_26 : PRL | None
        OBR.26 - Parent Result + (PRL) O S4.4.3.26

    obr_28 : list[XCN] | None
        OBR.28 - Result Copies To (XCN) O rep S4.4.3.28

    obr_29 : EIP | None
        OBR.29 - Parent Result Observation Identifier (EIP) O S4.4.3.29

    obr_30 : str | None
        OBR.30 - Transportation Mode (ID) O S4.4.3.30 | 0124 - Transportation Mode

    obr_31 : list[CWE] | None
        OBR.31 - Reason for Study (CWE) O rep S4.4.3.31 | 9999 - no table for CE

    obr_32 : NDL | None
        OBR.32 - Principal Result Interpreter + (NDL) O S4.4.3.32

    obr_33 : list[NDL] | None
        OBR.33 - Assistant Result Interpreter + (NDL) O rep S4.4.3.33

    obr_34 : list[NDL] | None
        OBR.34 - Technician + (NDL) O rep S4.4.3.34

    obr_35 : list[NDL] | None
        OBR.35 - Transcriptionist + (NDL) O rep S4.4.3.35

    obr_36 : str | None
        OBR.36 - Scheduled Date/Time + (DTM) O S4.4.3.36

    obr_37 : str | None
        OBR.37 - Number of Sample Containers * (NM) O S4.4.3.37

    obr_38 : list[CWE] | None
        OBR.38 - Transport Logistics of Collected Sample * (CWE) O rep S4.4.3.38 | 9999 - no table for CE

    obr_39 : list[CWE] | None
        OBR.39 - Collector's Comment * (CWE) O rep S4.4.3.39 | 9999 - no table for CE

    obr_40 : CWE | None
        OBR.40 - Transport Arrangement Responsibility (CWE) O S4.4.3.40 | 9999 - no table for CE

    obr_41 : str | None
        OBR.41 - Transport Arranged (ID) O S4.4.3.41 | 0224 - Transport Arranged

    obr_42 : str | None
        OBR.42 - Escort Required (ID) O S4.4.3.42 | 0225 - Escort Required

    obr_43 : list[CWE] | None
        OBR.43 - Planned Patient Transport Comment (CWE) O rep S4.4.3.43 | 9999 - no table for CE
    ...
    obr_48 : CWE | None
        OBR.48 - Medically Necessary Duplicate Procedure Reason (CWE) C S4.4.3.48 | 0476 - Medically Necessary Duplicate Procedure Reason

    obr_49 : CWE | None
        OBR.49 - Result Handling (CWE) O S4.4.3.49 | 0507 - Observation Result Handling

    obr_50 : CWE | None
        OBR.50 - Parent Universal Service Identifier (CWE) O S4.4.3.50

    obr_51 : EI | None
        OBR.51 - Observation Group ID (EI) O S4.4.3.51

    obr_52 : EI | None
        OBR.52 - Parent Observation Group ID (EI) O S4.4.3.52

    obr_53 : list[CX] | None
        OBR.53 - Alternate Placer Order Number (CX) O rep S4.4.3.53

    obr_54 : EIP | None
        OBR.54 - Parent Order (EIP) O S4.4.1.8
    """
    ...
    obr_13: Optional[List[CWE]] = Field(
        ...
        description=(
            "O | Item #00247 | Table 0916 - Relevant Clincial Information"
        ),
    )
    ...
    obr_30: Optional[str] = Field(
        ...
        description=(
            "O | Item #00262 | Table 0124 - Transportation Mode | LEN:4"
        ),
    )
    ...
    obr_41: Optional[str] = Field(
        ...
        description="O | Item #01032 | Table 0224 - Transport Arranged | LEN:1",
    )
    ...
    obr_49: Optional[CWE] = Field(
        ...
        description=(
            "O | Item #01647 | Table 0507 - Observation Result Handling"
        ),
    )
