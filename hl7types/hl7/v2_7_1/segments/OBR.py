...
class OBR(HL7Model):
    """Observation Request (S4.4.3).

    Attributes
    ----------
    obr_1 : str | None
        OBR.1 (opt) - Set ID - OBR (SI) S4.4.3.1
    ...
    obr_4 : CWE
        OBR.4 (req) - Universal Service Identifier (CWE) S10.6.4.3

    obr_7 : str | None
        OBR.7 (opt) - Observation Date/Time # (DTM) S4.4.3.7

    obr_8 : str | None
        OBR.8 (opt) - Observation End Date/Time # (DTM) S4.4.3.8

    obr_9 : CQ | None
        OBR.9 (opt) - Collection Volume * (CQ) S4.4.3.9

    obr_10 : list[XCN] | None
        OBR.10 (opt, rep) - Collector Identifier * (XCN) S4.4.3.10

    obr_11 : str | None
        OBR.11 (opt) - Specimen Action Code * (ID) S4.4.3.11 | 0065 - Specimen Action Code

    obr_12 : CWE | None
        OBR.12 (opt) - Danger Code (CWE) S4.4.3.12 | 9999 - no table for CE

    obr_13 : list[CWE] | None
        OBR.13 (opt, rep) - Relevant Clinical Information (CWE) S4.4.3.13 | 0916 - Relevant Clincial Information

    obr_16 : list[XCN] | None
        OBR.16 (opt, rep) - Ordering Provider (XCN) S4.4.1.12

    obr_17 : list[XTN] | None
        OBR.17 (opt, rep) - Order Callback Phone Number (XTN) S4.4.3.17

    obr_18 : str | None
        OBR.18 (opt) - Placer Field 1 (ST) S4.4.3.18

    obr_19 : str | None
        OBR.19 (opt) - Placer Field 2 (ST) S4.4.3.19

    obr_20 : str | None
        OBR.20 (opt) - Filler Field 1 + (ST) S4.4.3.20

    obr_21 : str | None
        OBR.21 (opt) - Filler Field 2 + (ST) S4.4.3.21

    obr_22 : str | None
        OBR.22 (opt) - Results Rpt/Status Chng - Date/Time + (DTM) S4.4.3.22

    obr_23 : MOC | None
        OBR.23 (opt) - Charge to Practice + (MOC) S4.4.3.23

    obr_24 : str | None
        OBR.24 (opt) - Diagnostic Serv Sect ID (ID) S4.4.3.24 | 0074 - Diagnostic Service Section ID

    obr_25 : str | None
        OBR.25 (opt) - Result Status + (ID) S4.4.3.25 | 0123 - Result Status

    obr_26 : PRL | None
        OBR.26 (opt) - Parent Result + (PRL) S4.4.3.26

    obr_28 : list[XCN] | None
        OBR.28 (opt, rep) - Result Copies To (XCN) S4.4.3.28

    obr_29 : EIP | None
        OBR.29 (opt) - Parent Result Observation Identifier (EIP) S4.4.3.29

    obr_30 : str | None
        OBR.30 (opt) - Transportation Mode (ID) S4.4.3.30 | 0124 - Transportation Mode

    obr_31 : list[CWE] | None
        OBR.31 (opt, rep) - Reason for Study (CWE) S4.4.3.31 | 9999 - no table for CE

    obr_32 : NDL | None
        OBR.32 (opt) - Principal Result Interpreter + (NDL) S4.4.3.32

    obr_33 : list[NDL] | None
        OBR.33 (opt, rep) - Assistant Result Interpreter + (NDL) S4.4.3.33

    obr_34 : list[NDL] | None
        OBR.34 (opt, rep) - Technician + (NDL) S4.4.3.34

    obr_35 : list[NDL] | None
        OBR.35 (opt, rep) - Transcriptionist + (NDL) S4.4.3.35

    obr_36 : str | None
        OBR.36 (opt) - Scheduled Date/Time + (DTM) S4.4.3.36

    obr_37 : str | None
        OBR.37 (opt) - Number of Sample Containers * (NM) S4.4.3.37

    obr_38 : list[CWE] | None
        OBR.38 (opt, rep) - Transport Logistics of Collected Sample * (CWE) S4.4.3.38 | 9999 - no table for CE

    obr_39 : list[CWE] | None
        OBR.39 (opt, rep) - Collector's Comment * (CWE) S4.4.3.39 | 9999 - no table for CE

    obr_40 : CWE | None
        OBR.40 (opt) - Transport Arrangement Responsibility (CWE) S4.4.3.40 | 9999 - no table for CE

    obr_41 : str | None
        OBR.41 (opt) - Transport Arranged (ID) S4.4.3.41 | 0224 - Transport Arranged

    obr_42 : str | None
        OBR.42 (opt) - Escort Required (ID) S4.4.3.42 | 0225 - Escort Required

    obr_43 : list[CWE] | None
        OBR.43 (opt, rep) - Planned Patient Transport Comment (CWE) S4.4.3.43 | 9999 - no table for CE

    obr_44 : CNE | None
        OBR.44 (opt) - Procedure Code (CNE) S17.4.1.14 | 0088 - Procedure Code

    obr_45 : list[CNE] | None
        OBR.45 (opt, rep) - Procedure Code Modifier (CNE) S17.4.1.15 | 0340 - Procedure Code Modifier

    obr_46 : list[CWE] | None
        OBR.46 (opt, rep) - Placer Supplemental Service Information (CWE) S10.6.4.11 | 0411 - Supplemental Service Information Values

    obr_47 : list[CWE] | None
        OBR.47 (opt, rep) - Filler Supplemental Service Information (CWE) S10.6.4.12 | 0411 - Supplemental Service Information Values

    obr_48 : CWE | None
        OBR.48 (opt) - Medically Necessary Duplicate Procedure Reason (CWE) S4.4.3.48 | 0476 - Medically Necessary Duplicate Procedure Reason

    obr_49 : CWE | None
        OBR.49 (opt) - Result Handling (CWE) S4.4.3.49 | 0507 - Observation Result Handling

    obr_50 : CWE | None
        OBR.50 (opt) - Parent Universal Service Identifier (CWE) S4.4.3.50

    obr_51 : EI | None
        OBR.51 (opt) - Observation Group ID (EI) S4.4.3.51

    obr_52 : EI | None
        OBR.52 (opt) - Parent Observation Group ID (EI) S4.4.3.52

    obr_53 : list[CX] | None
        OBR.53 (opt, rep) - Alternate Placer Order Number (CX) S4.4.3.53

    obr_54 : EIP | None
        OBR.54 (opt) - Parent Order (EIP) S4.4.1.8
    """
    ...
    obr_13: Optional[List[CWE]] = Field(
        default=None,
        validation_alias=AliasChoices(
            "obr_13",
        ...
        description="Item #247 | Table HL70916",
    )
    ...
    obr_29: Optional[EIP] = Field(
        ...
        title="Parent Result Observation Identifier",
    ...
    @field_validator("obr_7", "obr_8", "obr_22", "obr_36", mode='before')
    @classmethod
