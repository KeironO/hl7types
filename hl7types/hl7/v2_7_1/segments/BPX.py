...
class BPX(HL7Model):
    ...
    ----------
    bpx_1 : str
        BPX.1 - Set ID - BPX (SI) R S4.13.2.1

    bpx_2 : CWE
        BPX.2 - BP Dispense Status (CWE) R S4.13.2.2 | 0510 - Blood Product Dispense Status

    bpx_3 : str
        BPX.3 - BP Status (ID) R S4.13.2.3 | 0511 - BP Observation Status Codes Interpretation

    bpx_4 : str
        BPX.4 - BP Date/Time of Status (DTM) R S4.13.2.4

    bpx_5 : EI | None
        BPX.5 - BC Donation ID (EI) C S4.13.2.5

    bpx_6 : CNE | None
        BPX.6 - BC Component (CNE) C S4.13.2.6 | 9999 - no table for CE

    bpx_7 : CNE | None
        BPX.7 - BC Donation Type / Intended Use (CNE) O S4.13.2.7 | 9999 - no table for CE

    bpx_8 : CWE | None
        BPX.8 - CP Commercial Product (CWE) C S4.13.2.8 | 0512 - Commercial Product

    bpx_9 : XON | None
        BPX.9 - CP Manufacturer (XON) C S4.13.2.9

    bpx_10 : EI | None
        BPX.10 - CP Lot Number (EI) C S4.13.2.10

    bpx_11 : CNE | None
        BPX.11 - BP Blood Group (CNE) O S4.13.2.11 | 9999 - no table for CE

    bpx_12 : list[CNE] | None
        BPX.12 - BC Special Testing (CNE) O rep S4.13.2.12 | 9999 - no table for CE

    bpx_13 : str | None
        BPX.13 - BP Expiration Date/Time (DTM) O S4.13.2.13

    bpx_14 : str
        BPX.14 - BP Quantity (NM) R S4.13.2.14

    bpx_15 : str | None
        BPX.15 - BP Amount (NM) O S4.13.2.15

    bpx_16 : CWE | None
        BPX.16 - BP Units (CWE) O S4.13.2.16 | 9999 - no table for CE

    bpx_17 : EI | None
        BPX.17 - BP Unique ID (EI) O S4.13.2.17

    bpx_18 : PL | None
        BPX.18 - BP Actual Dispensed To Location (PL) O S4.13.2.18

    bpx_19 : XAD | None
        BPX.19 - BP Actual Dispensed To Address (XAD) O S4.13.2.19

    bpx_20 : XCN | None
        BPX.20 - BP Dispensed to Receiver (XCN) O S4.13.2.20

    bpx_21 : XCN | None
        BPX.21 - BP Dispensing Individual (XCN) O S4.13.2.21
    """
