...
class RXD(HL7Model):
    ...
    ----------
    rxd_1 : str
        RXD.1 (req) - Dispense Sub-ID Counter (NM) S4.A.5.1

    rxd_2 : CWE
        RXD.2 (req) - Dispense/Give Code (CWE) S4.A.5.2 | 0292 - Vaccines administered

    rxd_3 : str
        RXD.3 (req) - Date/Time Dispensed (DTM) S4.A.5.3

    rxd_4 : str
        RXD.4 (req) - Actual Dispense Amount (NM) S4.A.5.4

    rxd_5 : CWE | None
        RXD.5 (opt) - Actual Dispense Units (CWE) S4.A.5.5 | 9999 - no table for CE

    rxd_6 : CWE | None
        RXD.6 (opt) - Actual Dosage Form (CWE) S4.A.5.6 | 9999 - no table for CE

    rxd_7 : str
        RXD.7 (req) - Prescription Number (ST) S4.A.4.15

    rxd_8 : str | None
        RXD.8 (opt) - Number of Refills Remaining (NM) S4.A.4.16

    rxd_9 : list[str] | None
        RXD.9 (opt, rep) - Dispense Notes (ST) S4.A.5.9

    rxd_10 : list[XCN] | None
        RXD.10 (opt, rep) - Dispensing Provider (XCN) S4.A.5.10

    rxd_11 : str | None
        RXD.11 (opt) - Substitution Status (ID) S4.A.4.9 | 0167 - Substitution Status

    rxd_12 : CQ | None
        RXD.12 (opt) - Total Daily Dose (CQ) S4.A.1.23

    rxd_13 : LA2 | None
        RXD.13 (opt) - Dispense-to Location (LA2) S4.A.5.13

    rxd_14 : str | None
        RXD.14 (opt) - Needs Human Review (ID) S4.A.1.16 | 0136 - Yes/no Indicator

    rxd_15 : list[CWE] | None
        RXD.15 (opt, rep) - Pharmacy/Treatment Supplier's Special Dispensing Instructions (CWE) S4.A.4.21 | 9999 - no table for CE

    rxd_16 : str | None
        RXD.16 (opt) - Actual Strength (NM) S4.A.5.16

    rxd_17 : CWE | None
        RXD.17 (opt) - Actual Strength Unit (CWE) S4.A.5.17 | 9999 - no table for CE

    rxd_18 : list[str] | None
        RXD.18 (opt, rep) - Substance Lot Number (ST) S13.4.11.2

    rxd_19 : list[str] | None
        RXD.19 (opt, rep) - Substance Expiration Date (DTM) S4.A.1.16

    rxd_20 : list[CWE] | None
        RXD.20 (opt, rep) - Substance Manufacturer Name (CWE) S4.A.1.17 | 0227 - Manufacturers of Vaccines (code=MVX)

    rxd_21 : list[CWE] | None
        RXD.21 (opt, rep) - Indication (CWE) S4.A.1.19 | 9999 - no table for CE

    rxd_22 : str | None
        RXD.22 (opt) - Dispense Package Size (NM) S4.A.4.28

    rxd_23 : CWE | None
        RXD.23 (opt) - Dispense Package Size Unit (CWE) S4.A.4.29 | 9999 - no table for CE

    rxd_24 : str | None
        RXD.24 (opt) - Dispense Package Method (ID) S4.A.4.30 | 0321 - Dispense Method

    rxd_25 : list[CWE] | None
        RXD.25 (opt, rep) - Supplementary Code (CWE) S4.A.1.24 | 9999 - no table for CE

    rxd_26 : CWE | None
        RXD.26 (opt) - Initiating Location (CWE) S4.A.5.26 | 9999 - no table for CE

    rxd_27 : CWE | None
        RXD.27 (opt) - Packaging/Assembly Location (CWE) S4.A.5.27 | 9999 - no table for CE

    rxd_28 : str | None
        RXD.28 (opt) - Actual Drug Strength Volume (NM) S4.A.5.28

    rxd_29 : CWE | None
        RXD.29 (opt) - Actual Drug Strength Volume Units (CWE) S4.A.5.29 | 9999 - no table for CE

    rxd_30 : CWE | None
        RXD.30 (opt) - Dispense to Pharmacy (CWE) S4.A.5.30 | 9999 - no table for CE

    rxd_31 : XAD | None
        RXD.31 (opt) - Dispense to Pharmacy Address (XAD) S4.A.5.31

    rxd_32 : str | None
        RXD.32 (opt) - Pharmacy Order Type (ID) S4.A.5.32 | 0480 - Pharmacy Order Types

    rxd_33 : CWE | None
        RXD.33 (opt) - Dispense Type (CWE) S4.A.5.33 | 0484 - Dispense Type

    rxd_34 : list[XTN] | None
        RXD.34 (opt, rep) - Pharmacy Phone Number (XTN) S4.A.5.34
    """
