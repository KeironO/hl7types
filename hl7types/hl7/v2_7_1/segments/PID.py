...
class PID(HL7Model):
    ...
    ----------
    pid_1 : str | None
        PID.1 - Set ID - PID (SI) O S2.B.8.2.1

    pid_3 : list[CX]
        PID.3 - Patient Identifier List (CX) R rep S17.7.4.33

    pid_5 : list[XPN]
        PID.5 - Patient Name (XPN) R rep S2.B.8.2.5 | 0200 - Name Type

    pid_6 : list[XPN] | None
        PID.6 - Mother's Maiden Name (XPN) O rep S2.B.8.2.6
    ...
    pid_11 : list[XAD] | None
        PID.11 - Patient Address (XAD) O rep S2.B.8.2.11

    pid_13 : list[XTN] | None
        PID.13 - Phone Number - Home (XTN) O rep S2.B.8.2.13

    pid_14 : list[XTN] | None
        PID.14 - Phone Number - Business (XTN) O rep S2.B.8.2.14

    pid_15 : CWE | None
        PID.15 - Primary Language (CWE) O S2.B.8.2.15 | 0296 - Primary Language
    ...
    pid_18 : CX | None
        PID.18 - Patient Account Number (CX) O S2.B.8.2.18

    pid_21 : list[CX] | None
        PID.21 - Mother's Identifier (CX) O rep S2.B.8.2.21

    pid_22 : list[CWE] | None
        PID.22 - Ethnic Group (CWE) O rep S15.4.8.28 | 0189 - Ethnic Group

    pid_23 : str | None
        PID.23 - Birth Place (ST) O S2.B.8.2.23

    pid_24 : str | None
        PID.24 - Multiple Birth Indicator (ID) O S2.B.8.2.24 | 0136 - Yes/no Indicator

    pid_25 : str | None
        PID.25 - Birth Order (NM) O S2.B.8.2.25

    pid_26 : list[CWE] | None
        PID.26 - Citizenship (CWE) O rep S15.4.8.30 | 0171 - Citizenship

    pid_27 : CWE | None
        PID.27 - Veterans Military Status (CWE) O S2.B.8.2.27 | 0172 - Veterans Military Status

    pid_29 : str | None
        PID.29 - Patient Death Date and Time (DTM) O S2.B.8.2.29

    pid_30 : str | None
        PID.30 - Patient Death Indicator (ID) O S2.B.8.2.30 | 0136 - Yes/no Indicator

    pid_31 : str | None
        PID.31 - Identity Unknown Indicator (ID) O S2.B.8.2.31 | 0136 - Yes/no Indicator

    pid_32 : list[CWE] | None
        PID.32 - Identity Reliability Code (CWE) O rep S2.B.8.2.32 | 0445 - Identity Reliability Code

    pid_33 : str | None
        PID.33 - Last Update Date/Time (DTM) O S2.B.8.2.33

    pid_34 : HD | None
        PID.34 - Last Update Facility (HD) O S2.B.8.2.34

    pid_35 : CWE | None
        PID.35 - Species Code (CWE) C S2.B.8.2.35 | 0446 - Species Code

    pid_36 : CWE | None
        PID.36 - Breed Code (CWE) C S2.B.8.2.36 | 0447 - Breed Code

    pid_37 : str | None
        PID.37 - Strain (ST) O S2.B.8.2.37

    pid_38 : list[CWE] | None
        PID.38 - Production Class Code (CWE) O rep S2.B.8.2.38 | 0429 - Production Class Code

    pid_39 : list[CWE] | None
        PID.39 - Tribal Citizenship (CWE) O rep S2.B.8.2.39 | 0171 - Citizenship

    pid_40 : list[XTN] | None
        PID.40 - Patient Telecommunication Information (XTN) O rep S2.B.8.2.40
    """
