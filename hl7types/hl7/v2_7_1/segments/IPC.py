...
class IPC(HL7Model):
    """Imaging Procedure Control Segment (S4.4.6).
    ...
    ipc_2 : EI
        IPC.2 - Requested Procedure ID (EI) R S4.4.6.2

    ipc_3 : EI
        IPC.3 - Study Instance UID (EI) R S4.4.6.3

    ipc_4 : EI
        IPC.4 - Scheduled Procedure Step ID (EI) R S4.4.6.4

    ipc_5 : CWE | None
        IPC.5 - Modality (CWE) O S4.4.6.5 | 9999 - no table for CE

    ipc_6 : list[CWE] | None
        IPC.6 - Protocol Code (CWE) O rep S4.4.6.6 | 9999 - no table for CE

    ipc_7 : EI | None
        IPC.7 - Scheduled Station Name (EI) O S4.4.6.7

    ipc_8 : list[CWE] | None
        IPC.8 - Scheduled Procedure Step Location (CWE) O rep S4.4.6.8 | 9999 - no table for CE

    ipc_9 : str | None
        IPC.9 - Scheduled Station AE Title (ST) O S4.4.6.9
    """
