...
class MSH(HL7Model):
    ...
    ----------
    msh_1 : str
        MSH.1 (req) - Field Separator (ST) S2.16.9.1

    msh_2 : str
        MSH.2 (req) - Encoding Characters (ST) S2.16.9.2

    msh_3 : HD | None
        MSH.3 (opt) - Sending Application (HD) S2.16.9.3 | 0361 - Sending/receiving application

    msh_4 : HD | None
        MSH.4 (opt) - Sending Facility (HD) S2.16.9.4 | 0362 - Sending/receiving facility

    msh_5 : HD | None
        MSH.5 (opt) - Receiving Application (HD) S2.16.9.5 | 0361 - Sending/receiving application

    msh_6 : HD | None
        MSH.6 (opt) - Receiving Facility (HD) S2.16.9.6 | 0362 - Sending/receiving facility

    msh_7 : TS
        MSH.7 (req) - Date/Time Of Message (TS) S2.16.9.7

    msh_8 : str | None
        MSH.8 (opt) - Security (ST) S2.16.9.8

    msh_9 : MSG
        MSH.9 (req) - Message Type (MSG) S2.16.9.9 | 0076 - Message type

    msh_10 : str
        MSH.10 (req) - Message Control ID (ST) S2.16.9.10

    msh_11 : PT
        MSH.11 (req) - Processing ID (PT) S2.16.9.11

    msh_12 : VID
        MSH.12 (req) - Version ID (VID) S2.16.9.12 | 0104 - Version ID

    msh_13 : str | None
        MSH.13 (opt) - Sequence Number (NM) S2.16.9.13

    msh_14 : str | None
        MSH.14 (opt) - Continuation Pointer (ST) S2.16.9.14

    msh_15 : str | None
        MSH.15 (opt) - Accept Acknowledgment Type (ID) S2.16.9.15 | 0155 - Accept/application acknowledgment conditions

    msh_16 : str | None
        MSH.16 (opt) - Application Acknowledgment Type (ID) S2.16.9.16 | 0155 - Accept/application acknowledgment conditions

    msh_17 : str | None
        MSH.17 (opt) - Country Code (ID) S2.16.9.17 | 0399 - Country code

    msh_18 : list[str] | None
        MSH.18 (opt, rep) - Character Set (ID) S2.16.9.18 | 0211 - Alternate character sets

    msh_19 : CE | None
        MSH.19 (opt) - Principal Language Of Message (CE) S2.16.9.19

    msh_20 : str | None
        MSH.20 (opt) - Alternate Character Set Handling Scheme (ID) S2.16.9.20 | 0356 - Alternate character set handling scheme

    msh_21 : list[str] | None
        MSH.21 (opt, rep) - Conformance Statement ID (ID) S2.16.9.21 | 0449 - Conformance statements
    """
    ...
    msh_21: Optional[List[str]] = Field(
        ...
        description="Item #1598 | Table HL70449",
    )
