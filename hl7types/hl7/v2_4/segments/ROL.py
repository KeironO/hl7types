...
class ROL(HL7Model):
    ...
    ----------
    rol_1 : EI | None
        ROL.1 - Role Instance ID (EI) C S12.4.3.1

    rol_2 : str
        ROL.2 - Action Code (ID) R S12.4.4.1 | 0287 - Problem/goal action code

    rol_3 : CE
        ROL.3 - Role-ROL (CE) R S12.4.3.3 | 0443 - Provider role

    rol_4 : list[XCN]
        ROL.4 - Role Person (XCN) R rep S12.4.3.4

    rol_5 : TS | None
        ROL.5 - Role Begin Date/Time (TS) O S12.4.3.5

    rol_6 : TS | None
        ROL.6 - Role End Date/Time (TS) O S12.4.3.6

    rol_7 : CE | None
        ROL.7 - Role Duration (CE) O S12.4.3.7

    rol_8 : CE | None
        ROL.8 - Role Action Reason (CE) O S12.4.3.8

    rol_9 : list[CE] | None
        ROL.9 - Provider Type (CE) O rep S12.4.3.9

    rol_10 : CE | None
        ROL.10 - Organization Unit Type - ROL (CE) O S12.4.3.10 | 0406 - Organization unit type

    rol_11 : list[XAD] | None
        ROL.11 - Office/Home Address (XAD) O rep S15.4.6.11

    rol_12 : list[XTN] | None
        ROL.12 - Phone (XTN) O rep S15.4.6.10
    """
