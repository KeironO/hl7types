...
class MFA(HL7Model):
    ...
    ----------
    mfa_1 : str
        MFA.1 (req) - Record-Level Event Code (ID) S8.5.3.1 | 0180 - Record-level event code

    mfa_2 : str | None
        MFA.2 (opt) - MFN Control ID (ST) S8.5.3.2

    mfa_3 : TS | None
        MFA.3 (opt) - Event Completion Date/Time (TS) S8.5.3.3

    mfa_4 : CE
        MFA.4 (req) - MFN Record Level Error Return (CE) S8.5.3.4 | 0181 - MFN record-level error return

    mfa_5 : list[CE]
        MFA.5 (req, rep) - Primary Key Value - MFA (CE) S8.5.3.5 | 9999 - for unknown CE data elements
