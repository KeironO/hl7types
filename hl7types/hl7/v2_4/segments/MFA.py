...
class MFA(HL7Model):
    ...
    ----------
    mfa_1 : str
        MFA.1 - Record-Level Event Code (ID) R S8.5.3.1 | 0180 - Record-level event code

    mfa_2 : str | None
        MFA.2 - MFN Control ID (ST) C S8.5.3.2

    mfa_3 : TS | None
        MFA.3 - Event Completion Date/Time (TS) O S8.5.3.3

    mfa_4 : CE
        MFA.4 - MFN Record Level Error Return (CE) R S8.5.3.4 | 0181 - MFN record-level error return

    mfa_5 : list[CE]
        MFA.5 - Primary Key Value - MFA (CE) R rep S8.5.3.5 | 9999 - for unknown CE data elements
    ...
    mfa_5: List[CE] = Field(
        ...
        description=(
            "R | Item #01308 | Table 9999 - for unknown CE data elements"
        ),
    )
