...
class RXG(HL7Model):
    ...
    ----------
    rxg_1 : str
        RXG.1 - Give Sub-ID Counter (NM) R S4.14.7.1

    rxg_2 : str | None
        RXG.2 - Dispense Sub-ID Counter (NM) O S4.14.6.2

    rxg_3 : TQ
        RXG.3 - Quantity/Timing (TQ) R S7.4.1.27

    rxg_4 : CE
        RXG.4 - Give Code (CE) R S4.14.6.4 | 0292 - Vaccines administered

    rxg_5 : str
        RXG.5 - Give Amount - Minimum (NM) R S4.14.6.5

    rxg_6 : str | None
        RXG.6 - Give Amount - Maximum (NM) O S4.14.6.6

    rxg_7 : CE
        RXG.7 - Give Units (CE) R S4.14.6.7

    rxg_8 : CE | None
        RXG.8 - Give Dosage Form (CE) O S4.14.6.8

    rxg_9 : list[CE] | None
        RXG.9 - Administration Notes (CE) O rep S4.14.7.9

    rxg_10 : str | None
        RXG.10 - Substitution Status (ID) O S4.14.6.10 | 0167 - Substitution status

    rxg_11 : LA2 | None
        RXG.11 - Dispense-To Location (LA2) O S4.14.6.11

    rxg_12 : str | None
        RXG.12 - Needs Human Review (ID) O S4.14.6.12 | 0136 - Yes/no indicator

    rxg_13 : list[CE] | None
        RXG.13 - Pharmacy/Treatment Supplier's Special Administration Instructions (CE) O rep S4.14.6.13

    rxg_14 : str | None
        RXG.14 - Give Per (Time Unit) (ST) C S4.14.6.14

    rxg_15 : str | None
        RXG.15 - Give Rate Amount (ST) O S4.14.6.15

    rxg_16 : CE | None
        RXG.16 - Give Rate Units (CE) O S4.14.6.16

    rxg_17 : str | None
        RXG.17 - Give Strength (NM) O S4.14.6.17

    rxg_18 : CE | None
        RXG.18 - Give Strength Units (CE) O S4.14.6.18

    rxg_19 : list[str] | None
        RXG.19 - Substance Lot Number (ST) O rep S13.4.11.2

    rxg_20 : list[TS] | None
        RXG.20 - Substance Expiration Date (TS) O rep S4.14.7.16

    rxg_21 : list[CE] | None
        RXG.21 - Substance Manufacturer Name (CE) O rep S4.14.7.17 | 0227 - Manufacturers of vaccines (code=MVX)

    rxg_22 : list[CE] | None
        RXG.22 - Indication (CE) O rep S4.14.7.19
    """
